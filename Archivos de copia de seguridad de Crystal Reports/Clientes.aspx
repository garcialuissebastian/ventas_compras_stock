...
<%@ Page Title="" Language="C#" MasterPageFile="~/App/Master/HomeOnOaa.Master" enableEventValidation="false" AutoEventWireup="true" CodeBehind="Clientes.aspx.cs" Inherits="HardSoft.App.OAA.Clientes" %>
  ...
  <div class="col-xs-3" >
      ...
      <select id="cmbTipoOp" class="form-control">
          ...
          <option value="3">CUIT</option>
           <option value="6">Nombre Fantasía</option>
...
<asp:TextBox ID="TxtCuit" runat="server"  class="form-control  required"  Width="120px"     onKeyPress="return acceptNum(event)"  autocomplete="off"></asp:TextBox>
 ...
 <asp:TextBox ID="TxtNombre" runat="server" class="form-control  required "  ></asp:TextBox>
       ...
       <div class="form-group">
         <label for="TxtApe" class="control-label col-xs-4">Nombre Fantasía</label>
         <div class="col-xs-8">
...
Contacto
 ...
 <asp:TextBox ID="TxtEmpNomb" runat="server" class="form-control"  ></asp:TextBox>
...
<button id="btnModificar24" type="button" class="btn2 btn-info   pull-left" onclick="habPop()">
                      ...
                      success: function (result) {
                          ...
                          if (result.d != null) { 
                             ...
                             // $("#cmbAfip option[value=" + result.d.Iva + "]").attr("selected", true);
                              ...
                              $("#Main_TxtEmpWeb").val(result.d.Emp_Web);
                              $("#Main_TxtEmpNomb").val(result.d.Emp_Nomb);
  ...
  //               }
          ...
          function resultadoL(Resultado) {
              ...
              Tabla_Titulo = "<table   id='grilla1' class='table   table-hover table-striped' style='width: 100%;'><thead><tr> <th></th><th></th>  <th>Razón Social</th><th> Fantasía </th><th>N° Doc</th><th>Tel</th><th>Cel</th><th>Dirección</th><th>Mail</th></tr></thead><tbody>";
              $.each(Pacientes, function (index, pacientes) {
                  var m1 = '';
                  var verojo = 'display:none';
                  var vertext = '';
                  if (pacientes.Contac_Mail!='') {
                      m1 = ' ; ';
                      verojo = '';
                      vertext += pacientes.Contac_Mail+ ' ; ';
                  }
                  var m2 = '';
                  if (pacientes.Emp_Mail != '') {
                      m2 = ' ; ';
                      verojo = '';
                      vertext += pacientes.Emp_Mail  + ' ; ';
                  }

                  var m3 = '';
                  if (pacientes.Contac_Mail1 != '') {
                      m3 = ' ; ';
                      verojo = '';
                      vertext += pacientes.Contac_Mail1 + ' ; ';
                  }
                  var m4 = '';
                  if (pacientes.Contac_Mail2 != '') {
                      m4 = ' ; ';
                      verojo = '';
                      vertext += pacientes.Contac_Mail2 + ' ; ';
                  }
                 

                  Tabla_Datos = Tabla_Datos + "<tr ><td  style='width:30px'> <a class='tooltips' onclick='gvDel(" + pacientes.Id + ",this);' > <img src='/images/boton-eliminar.png' style='background-position:50%; background-repeat:no-repeat;cursor:pointer' /> <span>Eliminar</span></a></td> <td style='width:30px'> <a class='tooltips' onclick='PopMod(" + pacientes.Id + ",this);' > <img src='/images/boton-editar.png' style='background-position:50%; background-repeat:no-repeat;cursor:pointer' /> <span>Editar</span></a></td> <td>" + pacientes.Nombre + "</td><td>" + pacientes.Apellido + "</td><td>" + pacientes.Tipo_Doc + ':' + pacientes.Doc_No + "</td><td>" + pacientes.Emp_Tel + "</td><td>" + pacientes.Emp_Cel + "</td><td>" + pacientes.Calle + ' N° ' + pacientes.Numero + "</td><td>" + pacientes.Contac_Mail + m1 + pacientes.Emp_Mail + m2 + pacientes.Contac_Mail1 + m3 + pacientes.Contac_Mail2 + " <a class='tooltips' onclick='gvver(\"" + vertext  + "\");' > <img src='/images/ojo.png' style='background-position:50%; background-repeat:no-repeat;cursor:pointer; "+verojo+"' /> <span>Ver</span></a>  </td></tr>";
              }); ''
            ...
            function habilitarFrm() {
              ...
              $("#Main_TxtContactCel").prop('disabled', false);
                $("#Main_TxtContacMail").prop('disabled', false);
                 $("#Main_TxtEmpNomb").prop('disabled', false);
          ...
          function deshabilitarFrm() {
             ...
             $("#Main_TxtNombre").prop('disabled', true);
              ...
              $("#Main_TxtEmpNomb").prop('disabled', true);
